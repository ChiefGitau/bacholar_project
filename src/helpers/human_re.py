from src.helpers import analyzeppl, data_frame_helper
    ...
    for image in tqdm(image_data):
        ...
        if  not check_duplicate(image_url, data):
            ...
            if image_response['status'] == 200:
                response_data = human_response(image_response, image['url'], image['filePath'])
...
def human_response(response, path, generator):
    male = 0
    female = 0
    group_a = 0
    group_b = 0
    group_c = 0
    log_name_split = generator.split('/')
    ...
    return {'image_id': path,'generator': log_name_split[1], 'number': len(response['data']),
